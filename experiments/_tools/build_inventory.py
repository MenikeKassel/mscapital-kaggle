# -*- coding: utf-8 -*-
"""生成 docs/experiment-inventory.md — 全仓库资产清单 (Phase A 产物, 可重跑)."""
import os, subprocess, datetime

BASE = r"D:\mscapital-kaggle"
...
lines = []
lines.append("# MSCapital 实验资产清单 (Experiment Inventory)\n")
lines.append(f"> 生成: {datetime.date.today()} (仓库工程化整理 Phase A) | 重跑: `python experiments/_tools/build_inventory.py`")
lines.append("> 角色: 全仓库资产地图 — 判断每个文件是什么/属于哪个实验/是否在用\n")

# 顶层结构
lines.append("## 0. 顶层结构\n")
lines.append("```")
lines.append(run("ls -la --block-size=M --ignore=.git --ignore=.venv | tail -n +2").strip())
lines.append("```\n")

# scripts
lines.append("## 1. scripts/ — 实验脚本 (147 个 .py + 9 个子目录)\n")
lines.append("按编号段分类:\n")
lines.append("| 段 | 实验归属 | 文件数 |")
lines.append("| -- | -- | -: |")
seg = [
...
# src
lines.append("## 2. src/mscapital/ — 项目库 (正式 CLI + 模型/特征)\n")
lines.append("| 模块 | 角色 |")
lines.append("| -- | -- |")
mods = [
...
# tests
lines.append("## 3. tests/ — 13 个测试文件 (112 用例, 全部通过)\n")
lines.append("覆盖: clean baseline / RealMLP builder / retrieval 时序断言 / residual 泄漏 / protocol-v2 / path signature / revol-lite CLI。\n")

# docs
lines.append("## 4. docs/ — 报告与文档 (51 文件)\n")
lines.append("| 类别 | 文件 |")
lines.append("| -- | -- |")
docg = [
...
# output
lines.append("## 5. output/ — 实验产物 (41GB, gitignore)\n")
lines.append("| 族 | 内容 | 说明 |")
lines.append("| -- | -- | -- |")
outg = [
...
lines.append("## 7. 未分类/待处理\n")
lines.append("- `docs/dataset-sample-walkthrough-2026-08-15.docx` — 示例报告的 Word 版 (未跟踪, 待用户决定保留/删除)")
lines.append("- `scripts/p6_candidate_diag.py` — P6 候选测试集诊断脚本 (未跟踪, 建议入库)")
lines.append("- `output/submissions/submission_v9_*` (13) — 无 LB 记录, 待确认是否提交过")
lines.append("- `output/smoke_*`, `output/local_temp`, `output/logs` — 临时产物, 可清理")

open(os.path.join(BASE, "docs", "experiment-inventory.md"), "w", encoding="utf-8").write("\n".join(lines))
