...
try:
    ...
    _torch_version = _t0.__version__.split("+", 1)[0]
except Exception:
    _cap = (0, 0)
    _torch_version = "unknown"
if _cap[0] and _cap[0] < 7 and _torch_version != _target_torch:
    print(
        f"GPU capability {_cap} < 7.0 with torch {_torch_version}; "
        f"installing torch {_target_torch}...",
        flush=True,
    )
    subprocess.check_call([
        ...
        "install",
        "-q",
        f"torch=={_target_torch}",
        "numpy<2",
    ])
    os.execv(sys.executable, [sys.executable] + sys.argv)
if _cap[0] and _cap[0] < 7:
    print(f"P100-compatible torch {_torch_version} already active", flush=True)
...
import polars as pl
import pandas as pd
train = pl.read_parquet("/kaggle/input/msc-f0726-pq/f0726_train_f32.parquet").to_pandas().sort_values('sample_id')
...
test = pl.read_parquet("/kaggle/input/msc-f0726-pq/f0726_test_f32.parquet").to_pandas().sort_values('sample_id')
...
NUMS = [c for c in train.columns if c not in CATS + ['sample_id', target_col, 'month']]
...
# This kernel exists only to calibrate the PSEUDO fold.  Persist the best
# validation model immediately; the inherited full-train/test section below
# is intentionally not part of this job.
if best_model_state is None:
    raise RuntimeError("PSEUDO training produced no best model state")

named_parameters = dict(model.named_parameters())
missing_parameters = set(named_parameters) - set(best_model_state)
if missing_parameters:
    raise RuntimeError(
        f"Best EMA state is missing parameters: {sorted(missing_parameters)[:5]}"
    )
with torch.no_grad():
...
with torch.no_grad():
    for i in range(0, len(X_num_val_tensor), 2048):
        pred = model(
            X_num_val_tensor[i:i + 2048],
            X_cat_val_tensor[i:i + 2048],
            return_codes=False,
        ).mean(dim=1).squeeze()
...
print(
    f"PSEUDO pred saved from best EMA: {val_preds.shape}, "
    f"cos={cosine_similarity_score(val_preds, y_val):.6f}",
    flush=True,
)
print("PSEUDO_ONLY complete", flush=True)
sys.exit(0)
# %%
# ============ 7. 用全量数据重新训练（参数完全一致） ============
print("\n使用全量数据重新训练...")
