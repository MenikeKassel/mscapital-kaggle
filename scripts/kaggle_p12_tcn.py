...
def main():
    ...
    # per-channel 标准化 (train 统计量) + 防 NaN
    ...
    Xft = torch.from_numpy(Xf_tr)
    Xst = torch.from_numpy(Xs_tr)
    Xfv = torch.from_numpy(Xf_va)
    Xsv = torch.from_numpy(Xs_va)
    n = len(y_tr)
