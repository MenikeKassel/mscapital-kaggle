# -*- coding: utf-8 -*-
"""
Final v2: XGB + CatBoost + MLP-ens 三模型融合提交 (H1权重 0.1/0.4/0.5, LGBM权重0被淘汰)
全量训练(0-70月) + test预测
"""
import time
import random
...
N_THREADS = 12
OUT = r"D:\mscapital-kaggle\output\submissions\submission_blend_v2.csv"
W = {"xgb": 0.1, "cat": 0.4, "mlp": 0.5}
XGB_ITER = 815
CAT_ITER = 1823   # 1585 * 1.15
...
# --- 融合 ---
p_final = W["xgb"] * p_x + W["cat"] * p_c + W["mlp"] * p_m
...
print(f"pred: mean={sub['prediction'].mean():.6f} std={sub['prediction'].std():.6f} NaN={sub['prediction'].is_null().sum()}")
