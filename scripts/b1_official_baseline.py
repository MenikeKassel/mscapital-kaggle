# -*- coding: utf-8 -*-
"""
B1 锚点: 官方基线 lgb_baseline 本地复刻 (主办方 Rib~, LB 0.117)
改动: DATA路径/输出路径/num_threads=12/进度打印
其余逻辑与官方完全一致, 不调参。
"""
import os
import gc, time
...
DATA = r"D:\mscapital-forecasting\data\raw"
OUT_CSV = r"D:\mscapital-kaggle\output\submissions\b1_official_lgb.csv"
N_THREADS = 12
T0 = time.time()
...
def main():
    ...
    print(f"train months 0-50: {tr_df.height:,} | valid months 51-70: {va_df.height:,}", flush=True)
    del tr; gc.collect()
    ...
    print(f"submission saved: {OUT_CSV} ({len(ids_te):,} rows)", flush=True)
