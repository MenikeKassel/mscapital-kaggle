...
## 7. 消融实测结果 (2026-08-16, 全链完成)

> 协议: PSEUDO fold (train m0-32 / eval m33-70), MSE 训练 + 全局 cosine 选 checkpoint, 30 epochs。
> 锚点 C-05 = 0.113261 (best ep 12)。每步只动一个变量。实现忠实 pytabkit 源码。

| 项目 | 组件 | PSEUDO cos | Δ vs 锚 | 判定 | 解读 |
|---|---|---|---|---|---|
| C-05 | 冻结锚点 (robust+clip+const LR+ReLU) | 0.113261 | — | — | 复现锚 |
| C-06 | 无 robust+clip (StandardScaler) | 0.112136 | **−0.0011** | 🟡 | 预处理有价值 (论文 +9.5% 方向一致) |
| C-07 | β2 0.999→0.95 | 0.114185 | **+0.0009** | 🟡 | 方向一致, 量级远小于论文 +22.8% |
| C-08 | constant→cosine decay (1→0) | 0.112410 | **−0.0009** | 🔴 | 30ep 短训练下后期 LR 过低早停 (ep9); 论文 +13.5% 需 256ep+调 LR |
| C-09 | ReLU→Parametric Mish | 0.122404 | **+0.0091** | 🟢 | 大增益; ReLU 死区在 robust+clip 数据代价高 |
| C-10 | 无嵌入→PL (4维/特征) | 0.125215 | **+0.0120** | 🟢 | **最大组件**, 复现论文第一消融 (+20.6%) |
| C-11 | 无嵌入→PBLD | 0.122602 | **+0.0093** | 🟢 | 略低于 PL 0.0026 (论文 PL≈PBLD 预期) |
| C-12 | Learnable scaling layer | 0.112446 | **−0.0008** | 🔴 | 归一化后软特征选择无增量 |
| C-13 | dropout0.15+wd0.02 flat_cos | 0.119053 | **+0.0058** | 🟢 | 正则化节奏有效 (论文 constant 反而差复现) |
| C-14 | cosine→coslog4 (4周期) | 0.115357 | **+0.0021** | 🟢 | 周期重启有效; 论文 ns 我们更明显 |
| C-15 | NT 参数化+数据驱动 init | 0.075803 | **−0.0375** | 🔴 | 灾难: NTK 梯度尺度与 LR=1e-3 不匹配 (论文配 lr=0.2); best ep 28 仍在爬升 → 组件交互, 非独立 |

### 7.1 关键学习点 (大白话)

1. **表达力组件 > 优化组件**: 嵌入 (+0.012) 和参数化激活 (+0.009) 远大于 β2 (+0.0009)/调度。数据喂进去的"形状"比优化器细节重要。
2. **论文数字是"recipe 内敏感度"不是普适增益**: β2 论文 +22.8% 我们 +0.0009; cosine 论文 +13.5% 我们 −0.0009。**复现方向容易, 复现量级难**——协议/数据/LR 耦合。
3. **30ep 短训练改变组件排序**: cosine decay 在 256ep 有效, 30ep 有害 (后期 LR 过低); coslog4 反而 +0.0021 (周期重启在短训练也有效)。
4. **C-15 是组件交互教科书案例**: NT 参数化不是独立组件, 它重新标定梯度尺度, 必须与 LR 联合调整。单变量消融在"强耦合组件"上会误杀。
5. **下一步自然延伸: 正组件叠加** (β2=0.95 + ParamMish + PL + schedreg + coslog4 ≈ 论文完整配方), 验证增益可加性 (注意: 交互可能非线性)。
