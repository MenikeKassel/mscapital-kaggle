...
# 11. P4-06A / P4-07 执行结果 (2026-08-14)

## P4-06A 600s long-context residual probe → **不通过 (链条断裂)**

冻结 4×7=28 market-state 特征, M01-A 协议 4 outer:

| 帧 | PSEUDO Δ | H2 Δ | T3 Δ | T4 Δ | mean Δ | 正 outer |
|---|---:|---:|---:|---:|---:|:---:|
| short (仅最后60s, 7) | +0.00019 | +0.00100 | +0.00041 | +0.00084 | **+0.00061** | 4/4 |
| long (仅-600..-60s, 21) | **+0.00000** | +0.00003 | +0.00005 | +0.00021 | +0.00007 | 3/4 |
| both (28) | +0.00000 | +0.00117 | +0.00006 | +0.00039 | +0.00040 | 3/4 |

**关键结论**:
- **600s 长上下文能解释 LB142-v7 分歧 (R²=0.00117, 18.4× null), 但不能预测 y−v7 残差 (PSEUDO +0.0000)**
- 证据链: `600s → LB142-v7` ✅ 成立;`600s → y−v7` ❌ 断裂
- **长上下文是 LB142 模型行为的解释变量, 不是 target 的缺失信息源 → 按决策树不做 600s feature explosion**
- 附带发现: short (market 快照 60s) 4/4 正 (mean +0.0006) — 快照表示与事件流表示 (M01-A +0.0000) 有微弱互补, 但幅度未达正式门禁

## P4-07 0.5 群副线 → **关闭 (保留发现)**

- is_half_group 作为特征: ΔPSEUDO = 0.000000 (无增益)
- std(y|0.5)/std(y|main) 按月: mean 0.563, std 0.557, range [0.000, 3.142] → **非稳定 0.17, 不支持 instrument-specific scale**
- test 上 LB142/v7 预测幅度与主群一致 (ratio 0.96/1.02) → 两模型均未特殊处理该群
- 判定: 真实存在的结构异常 (train/test 稳定存在, 0.76%), 但无预测价值, 留作数据结构发现

## 假设状态更新 (命名校正)

- **LB142 long-context hypothesis**: 直接证据 (P4-01a) — LB142 确实使用长时域, 但对 target 无增量 (P4-06A)
- **原始 H4 (external/pseudo-label)**: 仍无证据 — 与 long-context 分开, 不再混淆
- **H5 (模型能力/训练方法) 概率上调**: LB142 的 factors 表 + cosine loss + grid 编码 60s 事件的方式可能是主要差异来源
- **600s 信息层**: 确认为 LB142-v7 差异解释变量, 排除为 target 信息源

---

# 12. P4-08A 受控 loss 消融 → **重大正结果 (cosine loss)**

同特征 (152) / 同架构 (单 MLP 256-256-64) / 同切分, 唯一变量 = loss:

| loss | PSEUDO Δ | H2 Δ | T3 Δ | T4 Δ | mean Δ | α |
|---|---:|---:|---:|---:|---:|---:|
| MSE | +0.0000 | +0.0000 | +0.0000 | +0.0000 | +0.0000 | 0.00 |
| **cosine (per-batch demean)** | **+0.00794** | +0.00589 | +0.00715 | +0.00715 | **+0.00703** | 0.09-0.17 |

**稳健性诊断**:
- corr(pred, baseline) = 0.58~0.70 (中度相关, 非重复信息)
- corr(pred, y) = 0.15~0.18 (单模型)
- **oracle-alpha 检查**: tune α 0.09-0.17 → oracle 0.30;delta(oracle) 0.012-0.019 > delta(tune) → tune 保守, 无过拟合
- 月度分散: 10/12, 16/20, 26/30, 26/30 正 (非集中少数月)
- **活动度分层: 高活动增益最大** (T3: hi +0.0138 vs lo +0.0035) — 与 LB142 分歧画像方向一致!

**解释链条**:
1. LB 指标 = 未中心化 cosine → MSE 训练天然错配 (月度 target 尺度变化 2.69×, MSE 偏重大波动月)
2. cosine loss (per-batch demean) 优化方向一致的目标 → 学到 baseline 缺失的方向信息
3. **LB142 源码 metrics.py 用 cosine_loss 训练不是巧合**
