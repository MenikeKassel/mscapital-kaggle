...
## P9-A-H14 多 seed 验证 (2026-08-20, 推翻单 seed regime 结论)

单 seed (2026) 曾显示 cancel 增益高度集中于 hi_act (hi +0.0103 / low −0.0064) → 判 YELLOW。
补 3 seed 受控实验 (seed 2026/2027/2028 × base 152 / raw 撤单 / mask 低撤单置0):

| seed | base frozen | raw Δ | mask Δ |
|---|---|---|---|
| 2026 | 0.114364 | **+0.004146** | −0.002590 |
| 2027 | 0.114607 | **+0.005986** | +0.004248 |
| 2028 | 0.110093 | **+0.011313** | +0.010799 |
| **均值** | — | **+0.007148** (3/3 正, std 0.0030) | +0.004152 (2/3 正) |

**raw 撤单增益多 seed 稳健** (3/3 正, 均值 +0.0071, 甚至高于单 seed 读数)。

**regime 分裂 (raw, by o_n_45)**: low_act Δ = −0.0064 / **+0.0105** / **+0.0073**; hi_act Δ = +0.0103/+0.0044/+0.0132。
→ 单 seed 2026 的 "low_act 拖累" 是 **seed 噪声**, 不是稳定 regime 效应; 均值上低/高活动
**两区皆正** (low +0.0038 / hi +0.0093)。

**mask (低撤单样本撤单列置0, q20 阈值 6700) — 机制猜想证伪**: mask−raw 三 seed 全 ≤ 0
(−0.0067/−0.0017/−0.0005), 且 mask 令 seed 2026 翻负。低撤单样本上撤单特征**不是噪声**——
强制清空反而丢了信号又没救回 low_act。判: ❌ 不重复 "hard-mask 低撤单样本" 修复路径。

### 修正后判定
- **P9-A (撤单侧拆不对称) = 稳健 GREEN 量级候选**: 3-seed 均值 frozen Δ **+0.0071**, 3/3 seed 全正,
  双 regime 均值皆正 → 从 YELLOW 升级为**进候选链/联合验证** (与 Z 同源且更聚焦, 是 Z 绿灯的候选主成分)。
- 保留诚实标注: 单次 best-epoch cosine 的 seed 方差大 (base 2028 0.1101 vs 2026 0.1144), 均值才是可靠估计。

## P9-A-V2 三方对比: cancel × Z (2026-08-20 → docs) — "直接用 A"被否决

C-05 协议, seed 2026, frozen 51-70:

| 臂 | frozen | Δ | pos |
|---|---|---|---|
| base 152 | 0.114364 | — | 20/20 |
| **cancel** (152+13) | 0.118510 | **+0.004146** | 20/20 |
| **Z** (152+73Z) | 0.123030 | **+0.008666** | 20/20 |
| **cancel+Z** (152+73Z+13) | 0.122216 | **+0.007852** | 20/20 |

- **替代判定: 否决** — A 单独 (+0.0041) 只有 Z (+0.0087) 一半强, 用 A 换 Z 会丢一半信号。
- **叠加判定: 否决** — J−Z = **−0.0008** (受控同 seed, 月度 Δ 正 10/20=掷硬币) → raw cancel 加到 Z 上零增益略有害。
- **归因闭合**: cancel ⊂ Z (Z 内含 Z_ob_cancel_side_imb 及其条件化形式), 撤单族确实是 Z 绿灯的驱动成分, 但 Z 的
  market/tx 条件化结构把这段信息表达得更好。**结论: "直接用 A" (替代或叠加) 均不成立; 生产资产仍是 152+73Z,
  A 的价值是归因证据 + Z 的可解释成分, 不作为独立生产特征。**
