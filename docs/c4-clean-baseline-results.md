...
The uniform production rule was separately applied to all four outers with
each fold's inner-learned RMS scales:

| Outer | Production score | Delta vs RealMLP |
|---|---:|---:|
| PSEUDO | 0.142357676 | +0.005977758 |
| H2 | 0.141795046 | +0.003507166 |
| T3 | 0.143515387 | +0.003729854 |
| T4 | 0.156923672 | +0.006629838 |
| Arithmetic mean | **0.146147945** | **+0.004961154** |

The predeclared default gate passes: all four outers are non-degrading and the
mean improvement is greater than `+0.0005`.
...
- `predictions.npz` stores the uniform production-rule prediction;
  fold-adaptive predictions are clearly separated as diagnostics.
