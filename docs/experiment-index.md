...
| 当前生产基线 | C4 frozen: `0.63×RMS(RealMLP) + 0.37×RMS(Clean Table)` (0.142649/0.141762/0.143515/0.156924) |
...
| B1_ab | 特征组消融 | 窗口 -0.0102 | GREEN | 窗口统计绝对主力 |
| B2 | 特征精简 | -0.0000 | GREEN | 84 可采纳 |
| C1_feat | 增强窗口特征 | -0.0004 | RED | **特征工程到顶 (90 甜点位)** |
| D1_hp | 超参单变量 | +0.0006 | YELLOW | leaves64 采纳 |
| E1_tw | 时间衰减加权 | 全无效 | RED | **不再试时间衰减** |
| F1_mlp | 轻量 MLP | +0.0019 | YELLOW | 表格 NN 温和超越 |
| G1/G2/G3 | 融合链 | 0.1370→0.1389 | GREEN | 融合是最大杠杆 |
| H1_5m | 五模型 | +0.0004 | RED | **表格路线 0.122 平台期** |

提交: **v1 0.122** (#79) → v2 0.122 → v3 0.122
...
| P0.5-B | MLP Fairness | +0.0050 | SUPERSEDED | last-epoch 评估 bug 修正 |
| P0.5-C | R2 归一化干预 | +0.0023 (4/4) | GREEN | **归一化延长 alpha 寿命** |
| P0.5-D | R2 × 全融合 | +0.0016 | GREEN | 兑现 |

提交: **v4 0.123** (R2+temporal, 科研闭环验证成功)

## P1 表示与序列 (2026-08-11/12)

| ID | 实验 | 结果 | 状态 | 核心结论 |
| -- | -- | -: | -- | ---- |
| P1-01a | 22 微观 primitive | 构建完成 | GREEN | 事件流微观结构特征 |
| P1-01b | 双轴筛选 | alpha +0.0022 | YELLOW | 部分特征携带漂移 ⚠️ |
| P1-01c | 全融合 (R2+micro) | PSEUDO +0.0036 | GREEN | 迄今最大单步提升 |
| P1-01e | 相对化第二轮 | PSEUDO -0.0012 | RED | 第一轮已最优 |
| P1-02 | TCN 双塔 | 融合 3 折全正 | RED | **test 分布外灾难 (v6 0.082)**; 序列模型不可信 |

提交: **v5 0.125** (R2+22微观) → **v6 0.082** (TCN, N005 灾难) → **v7 0.135** (RealMLP 复刻 0.8/0.2) → **v8a 0.139** → **v8b 0.142** (#30, +lb142 0.5/0.5)

## P2 校准 (2026-08-12, Codex)

| ID | 实验 | 结果 | 状态 | 核心结论 |
| -- | -- | -: | -- | ---- |
| P2-cal | RealMLP PSEUDO 定标 | 0.138560; v7 融合 0.139683 | GREEN | Regime B gap 0.0047; 尺度门禁 0.7447 |

## C 系列 Clean Baseline v2 (2026-08-13)

| ID | 实验 | 结果 | 状态 | 核心结论 |
| -- | -- | -: | -- | ---- |
| C1 | Clean RealMLP-v2a | ~0.143 | GREEN | 干净复刻基线 |
| C2 | RealMLP 消融 | 30ep 最优 | GREEN | 30-epoch 生产方案 |
| C3 | Clean Table v2 | PSEUDO 0.135051 | GREEN | 与 legacy v5 一致 (+0.00018) |
| C4 | Clean Baseline v2 冻结 | +0.0050 mean | GREEN | **生产规则冻结**; 不得回改 |
...
| P3-02 | 状态条件化 | +0.00095 | RED | 拼接稀释 E01 信号 |
...
| P4-06A | 长残差 | +0.0000 | RED | 负面链 #1 |
| P4-07 | halfgroup 分层 | 分层证据 | YELLOW | 高波动方向质量差 |
| P4-08A | loss ablation | 验证偏差 | INVALID | **消融必须严格 nested** |
| P4-H1H2 | 资产/时间身份 | 无证据 | RED | H1+H2 关闭 |
| P4-LB142 | 分歧取证 | 高波动集中 | YELLOW | 外部模型信息面线索 |
| P4-MH | market history 特征 | ~+0.0005 | YELLOW | 600s 特征弱正 |

## M 系列残差表示 (2026-08-13) — 全部 RED

| ID | 实验 | 结果 | 状态 | 核心结论 |
| -- | -- | -: | -- | ---- |
| M01-A | Event Flow | ~0 | RED | 无残差增量 |
| M02/M02-T | LOB 几何 | ~0 | RED | 无残差增量 |
| M03 | Path Signature | ~0 | RED | 无残差增量 |
| M04 | Optiver 交互 | ~0 | RED | 无残差增量 |
| M05 | 状态 KNN | ~0 | RED | 无残差增量 (P6R 前身) |
| M06 | 截面审计 | 无截面 alpha | RED | 截面不可用 |

## E 系列状态条件化 (2026-08-13)

| ID | 实验 | 结果 | 状态 | 核心结论 |
| -- | -- | -: | -- | ---- |
| E01 | ReVol-lite | +0.0011 | YELLOW | 未过 gate (上界参考) |
| E02 | Reconditionor-lite | 窗口内 cos 0.013 | RED | **窗口内可解释 ≠ 跨月可预测** (负面链 #2) |
| E03 | 稳定性审计 | 稳健 | GREEN | 结论稳健 |
...
| P5-02I | 信息审计 (五条实锤) | \|y\| corr **0.466** | GREEN | **幅度富矿; 方向微弱 (AUC 0.564)**; GPT1 猜想反转 |
| P5-A | MAG-Gate | -0.000146 | RED | **幅度门控失败**; 嵌套后 gate≈常数 |
| P5-B | SCFI (LGB) | **+0.0075** (17/20 月) | GREEN | 条件创新 LGB 强 / NN 无 |
| P5-C | RICS 几何 | ≤0.011 | RED | 短窗形态无信息; 相位破坏反演不变 |
| P5-D | SCFI 生产验证 | +0.000849 | GREEN | 跨 learner 双确认第一步 |
| P5-E | RealMLP spot-check | **+0.0040** / blend +0.0014 | GREEN | **SCFI 升级门禁通过**; SmallMLP 代理推翻 |
...
| ID | 实验 | 结果 | 状态 | 核心结论 |
| -- | -- | -: | -- | ---- |
| P7-AMP | 幅度门控快速版 | +0.00000 | RED | **α 单调=0; 高波动方向质量差 (0.215→0.11)**; 幅度路线整体关闭 |
...
| 幅度门控/加权 | ❌ 已证伪 | P5-A + P7-AMP 双重确认 |
| 残差建模 (均值/检索) | ❌ 已证伪 | M 系列 + P4-06A + P6R 五连杀 |
| 短窗形态 (shapelet/谱) | ❌ 已证伪 | P5-C RICS |
| 掩码预训练 / SAE latent | ❌ 已证伪 | P3-01/03 |
| 月度漂移预测 | ❌ 已证伪 | P4-05 决定性负结果 |
| 条件创新 SCFI | 🟢 **有效** | P5-B/D/E 三级确认, P6 候选在手 |
