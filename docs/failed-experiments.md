...
**特征工程到顶**: C1_feat (109 特征 -0.0004), B2 (删 EWM 持平)
...
**短窗跨通道形态无信息**: P5-C RICS (全层 ≤0.011 corr_y, R4 相位 -0.006)
...
**残差可解释性 ≠ 残差可预测增量** (五连杀): E02 (窗口内 cos 0.013), P4-06A (+0.0000), P5-02I resid probe (AUC 0.51), P5-03, **P6R-00** (最大 +0.000588 = gate 39%)
- 窗口内可解释的残差结构无法跨月变现为预测增量
- ❌ 不重复: 不做残差均值/检索-残差预测路线

**状态条件化拼接稀释**: P3-02 (+0.00095 四折全正但未过 gate)
- 简单拼接稀释纯状态信号 (E01 的 +0.0011 是上界)
- ❌ 不重复: 简单 concat 条件化; 条件化必须 FiLM 式或特征层创新 (SCFI 已替代)

## 3. 本地提升来自验证偏差

**P4-08A loss ablation**: 本地 cosine loss 增益来自验证集重用 → **INVALID**
...
**MLP last-epoch 评估 bug**: P0.5-B (评估协议高估 MLP 劣势)
- ✅ 教训: 评估必须 best-state 而非 last-epoch

## 4. 改善 |y| 估计, 但损害方向 (幅度侧整体关闭)

**P5-A MAG-Gate**: 嵌套 Δ = -0.000146 (非嵌套 +0.000138 假增益)
- 机制: gate≈常数 (std=0.011); 月度 6/20; 置换对照≈0
- ❌ 不重复: 4-bin/幂族幅度 gate; α∈[0,1] 细扫

**P7-AMP (GPT1 预注册终裁)**: Δ = +0.00000 (α* 全 0.0, 单调下降)
...
| 机制 | 数量 | 代表 |
| -- | -: | -- |
| 1. 没有新增信息 | 6 | C1_feat, P3-01/03/04, P5-C, B2 |
| 2. 有信息但无法兑现 | 6 | TCN, E02, P4-06A, P6R-00, P3-02, M 系列×6 |
| 3. 验证偏差 | 3 | P4-08A, P0-01, P0.5-B |
| 4. 幅度有害 | 4 | P5-A, P7-AMP, E1_tw, P1-01e |
| 5. 高度同质 | 3 | P1-01e, P0-03, H1_5m |
| 6. 信息面不存在 | 3 | P4-05, P4-H1H2, M06 |
