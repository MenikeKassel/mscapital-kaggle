...
## P10-RQ 生产提交 (2026-08-16 → docs/p10-rq-prod-submission-2026-08-16.md)

- **PSEUDO 0.143758 (γ=1.0, 历史最高) → LB 0.116 (远低于 v8b 0.142)** — 生产外推失败
- **根因 (如实)**: 定标模型 = arm_C (m0-32 训练), 提交模型 = RQ 全量 (0-70 训练), 两个模型; γ 跨模型套用无依据; 全量模型 test 表现从未被 PSEUDO 验证
- 提交格式事故: 列名 `target` → `prediction` (ref 55543480 ERROR → 55543658 COMPLETE)
- **教训**: PSEUDO 高分 ≠ 生产可提交; 生产模型必须与定标模型同源; 全量训练需留验证窗口
- 建议: 回到 P6 已验证的 RealMLP-C 生产候选 (PSEUDO 0.139248 / blendΔ +0.0014)

## P9-Lite 三探针 Gate (2026-08-20 → docs/p9-lite-results.md)

| 探针 | 特征 | Δ frozen 51-70 | pos 51-70 | 判定 |
|---|---|---|---|---|
| **A Cancel** (side-split 撤单压力) | +13 | **+0.004146** | 20/20 聚合 / 13/20 月Δ | 🟡 强信号但 **regime 集中**: hi_act +0.0103 / low_act −0.0064 → 非 clean GREEN, 进联合/校准 |
| **B Event-Time** (raw iat/burst) | +18 | **−0.003887** | 4/20 月Δ | 🔴 RED (152 基线已含事件节奏, 原始聚合冗余有害; Z 式条件化才兑现) |
| **C M55-lite** (DWI+entropy) | +10 | **+0.000542** | 12/20 月Δ | 🟡 边缘 → 进联合 |

- 基线复现: 各 base 臂 frozen 0.114364 / eval 0.113261 (与 C-05 逐位一致 ✓)
- **归因推论**: Z (152+73Z, +0.0060) 绿灯中, 撤单侧拆不对称很可能是主要份额 (P9-A 单测即 +0.0041 且同源); 事件节奏信息存在于 Z 的条件化形式, 不在原始聚合

## P9-A-H14 多 seed 验证 (2026-08-20 → docs/p9-lite-results.md)

| seed | base frozen | raw 撤单 Δ | mask Δ (低撤单置0) |
|---|---|---|---|
| 2026 | 0.114364 | **+0.004146** | −0.002590 |
| 2027 | 0.114607 | **+0.005986** | +0.004248 |
| 2028 | 0.110093 | **+0.011313** | +0.010799 |
| 均值 | — | **+0.007148 (3/3 seed 正)** | +0.004152 (2/3) |

- **撤单假设稳健确认**: 3-seed 均值 frozen Δ +0.0071 (GREEN 量级); 单 seed 2026 "regime 集中于 hi_act
  (low −0.0064)" 实为 seed 噪声 — raw 3-seed low_act Δ = −0.006/+0.011/+0.007, hi_act 全正×3, 双区均值皆正
- **mask 机制猜想证伪 (H14-V1)**: 低撤单样本撤单列置0 → mask−raw 三 seed 全 ≤ 0, 且 2026 翻负 → hard-mask 路径不重复
- **判定更新**: P9-A 从 YELLOW 升级为 **GREEN 候选 → 进候选链/联合验证** (与 Z 同源, 是 Z 绿灯候选主成分)

## P9-A-V2 三方对比: cancel × Z (2026-08-20, seed 2026 frozen 51-70 → "直接用 A"否决)

| 臂 | frozen | Δ | 判定 |
|---|---|---|---|
| base 152 | 0.114364 | — | |
| **cancel** (+13) | 0.118510 | +0.004146 | 单独有效, 但只有 Z 一半 |
| **Z** (+73Z) | 0.123030 | **+0.008666** | 生产绿灯线更强 |
| **cancel+Z** (+73Z+13) | 0.122216 | +0.007852 | **J−Z = −0.0008** (月度Δ正 10/20) → 无叠加增益 |

- **结论**: 替代否决 (A 只有 Z 一半强)、叠加否决 (J−Z −0.0008)。撤单族 ⊂ Z (Z 含 market/tx 条件化的
  Z_ob_cancel_side_imb), 是 Z 绿灯的驱动成分但 Z 表达得更好 → **撤单族不做独立生产特征; 生产资产仍 152+73Z**。
- 意义: P9-A 的价值 = 归因证据 + 回答了 brief 的"流动性撤退是否被基线丢失" (是, 但被 Z 补上了)。

## P6-R20260821 Z 线生产协议重跑 (2026-08-21, 今日验证数字 → 已提交)

> 用户拍板: 重跑 Z 线生产协议 (RealMLP-C 152+73Z 同源校准 + test 门禁) 拿今日验证数字, 过门禁后提交。
> 脚本: scripts/p6_prod_realmlp.py (全链路 2169s, GPU 4060Ti); 提交 ref 55656536 (w=0.55 候选)。

| 指标 | 今日重跑 | 8/15 (P6) | 对照 |
|---|---|---|---|
| RealMLP-C standalone (PSEUDO 33-70) | **0.139248** | 0.139248 | 逐位一致 ✓ (确定性复现) |
| canonical PSEUDO | 0.142550 | 0.142550 | 一致 ✓ |
| **blendΔ** (w tuned 21-32) | **+0.001460** | +0.001435 | 今日略高 (+0.00003) |
| blend w (PSEUDO tune 21-32) | 0.70 | 0.75 | 网格内差异, 敏感带内 |
| std_test/std_valid | **0.9726** | 0.9726 | 一致 ✓ |

**三窗口 w 敏感性** (今日 fresh 预测): 峰值 51-60 w=0.55 / 61-70 w=0.65 / PSEUDO w=0.55,
w=0.55 全窗口正且近峰值 → **生产 blend w=0.55** (与 8/15 生产调权一致)。
test 侧 corr(RealMLP-C, v8b) = 0.9280 (本地 61-70 0.940, 结构一致 ✓)。

- **提交候选**: `output/p6_prod/submission_candidate_p6_w055.csv` = v8b + 0.55×RealMLP-C(152+73Z)
- **实际结果**: 提交 ref 55656536 → **Public LB 0.141** (vs v8b 锚 0.142, **−0.001** — 本地 PSEUDO blendΔ +0.0014 未兑现到 LB, 如实记录负向)。期望 0.1423~0.1428 落空。
- **复盘要点**: blend 增益跨窗口稳定 (PSEUDO/61-70 双窗 +0.0014) 但 LB 仍降 — 与 8/15 P6 同候选未提交时的预期一致风险: v8b 已含 RealMLP 族 + lb142 外部成分, 再叠加 RealMLP-C 的增量被 LB 侧压缩/翻转。0.141 仍高于 P10 灾难 (0.116), 生产纪律 (同源校准+test 门禁) 有效, 增量为负 = 组件在 leaderboard 侧无净增益, 不追加投入。

## P6-ORIG 纯原创候选 (2026-08-21, 去 lb142)

> 用户拍板: 不用 lb142, 坚持原创。验证今天思路 (Z 线 152+73Z) 在纯原创锚上是否成立。
> 纯原创锚 = v5表格 (R2+22micro, PSEUDO 0.134871, 不含 lb142/RealMLP)。
> 全部用已有 PSEUDO/test 预测, 不重训。

**验证结论 (PSEUDO eval 33-70):**
| 臂 | w=0.20 Δ | best Δ | best w |
|---|---|---|---|
| v5tbl + RealMLP(152) [v7式] | +0.003230 | +0.005912 | 0.60 |
| **v5tbl + RealMLP-C(152+73Z) [今天思路]** | **+0.003381** | **+0.006239** | 0.60 |

- **今天思路在纯原创锚上成立 ✓**: Z 线叠加正且优于 RealMLP(152) (w=0.20 差 +0.00015, best 差 +0.0003)。
- **关键: 纯原创锚上 Z 线增量 (+0.0034~+0.0062) 远高于 v8b 锚 (+0.0014)** → 印证 lb142 的 RealMLP v9/v10 稀释了 Z 线增量, 用户去掉 lb142 的判断正确。
- w 敏感性: PSEUDO best 1.55 (边界, 过拟合风险); 分1 33-50 best 2.0 / 分2 51-70 best 1.05 — 权重大小不稳, 取稳健 **w=0.60** (全期 +0.0062 / 51-70 +0.0050)。

- 提交候选 (纯原创): `output/p6_prod/submission_orig_p6.csv` = v5表格 + 0.60×RealMLP-C(152+73Z)
- 门禁: RealMLP-C 本体 std ratio 0.9726 ✓ / corr(vs v8b) 0.939 / finite ✓
- 提交 ref 55657080 → **Public LB 0.132** (vs v8b 0.142 −0.010; vs v7 0.135 −0.003; 高于 v5 表格 0.125)
- **诚实结论**: 纯原创路线本地 PSEUDO blendΔ 兑现为 LB 增益低于预期 — 0.132 仍是纯原创最高的 (超 v5 0.125), 但输给含 lb142 的 v8b 0.142 达 −0.010。lb142 外部成分的价值 = 原原创基准 0.132~0.135 之上再加 ~0.010。原创路线的独立能力确认为 **0.132 量级**, 待后续组件叠加验证。
